...
export class AntigravityServer {
    ...
    private configureRoutes() {
        ...
        router.get('/task', (_req, res) => {
            const brainDir = path.join(os.homedir(), '.gemini', 'antigravity', 'brain');
            const candidates = ['task.md.resolved', 'task.md'];
            ...
            res.status(404).json({ error: 'Task file not found', searched: brainDir });
        });

        router.get('/walkthrough', (_req, res) => {
            const brainDir = path.join(os.homedir(), '.gemini', 'antigravity', 'brain');
            const candidates = ['walkthrough.md.resolved', 'walkthrough.md', 'tutorial.md.resolved', 'tutorial.md'];
            for (const name of candidates) {
                const filePath = this.findBrainFile(name);
                if (filePath) {
                    try {
            ...
            res.status(404).json({ error: 'Walkthrough file not found', searched: brainDir });
        });
        ...
        // ── ElevenLabs Proxy ─────────────────────────────────────────────────────
        // Proxies requests to ElevenLabs API to avoid browser CORS restrictions.
        // The API key is passed in the x-el-key header from the client.

        router.get('/el/voices', (req, res) => {
            const apiKey = req.headers['x-el-key'] as string;
            if (!apiKey) return res.status(400).json({ error: 'Missing x-el-key header' });
            ...
            const elReq = https.request(options, (elRes) => {
                let body = '';
                elRes.on('data', (chunk) => { body += chunk; });
                elRes.on('end', () => {
                    res.status(elRes.statusCode || 200).json(JSON.parse(body));
                });
            });
        ...
        router.post('/el/tts/:voiceId', (req, res) => {
            const apiKey = req.headers['x-el-key'] as string;
            ...
            const elReq = https.request(options, (elRes) => {
                res.status(elRes.statusCode || 200);
        ...
        router.get('/instances', async (_req, res) => {
            try {
                ...
                res.json({
                    ...
                    instances: instances.map(i => ({ id: i.id, port: i.port, url: i.url, title: i.title, isV2: i.isV2 }))
                });
            } catch (e) {
        ...
        router.get('/workspaces', async (_req, res) => {
            try {
                ...
                const workspaces = instances.map(i => {
                    const match = i.title.match(/^([^-]+)\s+-/);
                    return match ? match[1].trim() : i.title;
                });
                const unique = [...new Set(workspaces)];
                res.json({ workspaces: unique, instances });
            } catch (e) {
                res.status(500).json({ error: (e as Error).message });
            }
        });
        ...
        router.post('/orchestrate', async (req, res) => {
            const body = req.body;
            if (!body.targetWorkspace) return res.status(400).json({ error: 'targetWorkspace required' });
            
            try {
                const busDir = path.join(os.homedir(), '.gemini', 'antigravity', 'bus');
                if (!fs.existsSync(busDir)) fs.mkdirSync(busDir, { recursive: true });
                
                let workspaces = [body.targetWorkspace];
                if (body.targetWorkspace === 'ALL') {
                    const instances = await discoverInstances();
                    workspaces = instances.map(i => {
                        const m = i.title.match(/^([^-]+)\s+-/);
                        return m ? m[1].trim() : i.title;
                    });
                }

                const uniqueWorkspaces = [...new Set(workspaces)];
                for (const ws of uniqueWorkspaces) {
                    const filename = `${Date.now()}-${ws.replace(/[^a-zA-Z0-9]/g, '_')}-${Math.random().toString(36).slice(2)}.json`;
                    const payload = { ...body, targetWorkspace: ws, timestamp: Date.now(), source: 'orchestrator' };
                    // Default type to chat if not provided
                    if (!payload.type) payload.type = 'chat';
                    fs.writeFileSync(path.join(busDir, filename), JSON.stringify(payload));
                }
                
                res.json({ success: true, method: 'bus_queue', targetWorkspaces: uniqueWorkspaces });
            } catch (e) {
                res.status(500).json({ error: (e as Error).message });
            }
        });
        ...
        router.get('/conversations', async (_req, res) => {
            const brainDir = path.join(os.homedir(), '.gemini', 'antigravity', 'brain');
            try {
                if (!fs.existsSync(brainDir)) return res.json({ conversations: [] });
                const dirs = fs.readdirSync(brainDir, { withFileTypes: true }).filter(d => d.isDirectory() && d.name.length > 20 && d.name !== 'tempmediaStorage');
                const convs = dirs.map(d => {
                    const metaPath = path.join(brainDir, d.name, 'metadata.json');
                    let title = "Conversación " + d.name.substring(0,6);
                    let modified = 0;
                    try {
                        const stat = fs.statSync(path.join(brainDir, d.name));
                    ...
                    return { id: d.name, title, modified };
        ...
        // Fleet Control Multi-Agent API
        router.post('/api/agents/command', async (req, res) => {
            try {
                const { targetInstanceId, command, agentSource } = req.body;
                console.log(`[Fleet Control] Comando recibido desde ${agentSource} para la instancia ${targetInstanceId}:`, command);
                // Intercomunicador para que agentes Antigravity controlen otras instancias
                res.json({ success: true, message: `Command dispatched to fleet instance ${targetInstanceId}` });
            } catch (e) {
                res.status(500).json({ error: (e as Error).message });
            }
        });
        ...
        router.get('/debug/targets', async (_req, res) => {
            try {
                ...
                res.json({
                    ...
                    instances: instances.map(i => ({ id: i.id, port: i.port, title: i.title, url: i.url, isV2: i.isV2 }))
                });
            } catch (e) {
                res.status(500).json({ error: (e as Error).message });
            }
