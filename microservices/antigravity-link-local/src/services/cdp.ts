...
function getDiscoveryPorts(): number[] {
    ...
    try {
        const output = execSync('netstat -ano | findstr LISTENING | findstr 127.0.0.1', { encoding: 'utf-8' });
        ...
        console.error("Failed to discover dynamic ports via netstat:", e);
    }
    
    return Array.from(new Set([...staticPorts, ...dynamicPorts]));
}
...
export async function discoverInstances(): Promise<CDPInfo[]> {
    ...
    const listsByPort = await Promise.all(
        getDiscoveryPorts().map(async (port) => {
            try {
